#!/usr/bin/env perl
BEGIN {push @INC, '/afs/.genomecenter.ucdavis.edu/software/mmarge/1.0/lssc0-linux//bin'}
use warnings;
...
$_ = 0 for my ($sam, $peak, $tag, $help, $allele, $bed, $bismark, $bedpe, $hic, $keep_alleles, $coord, $coord2, $gtf, $paired);
...
sub printCMD {
	...
	print STDERR "\t-paired: file is paired-end\n";
...
for(my $i = 0; $i < @strains; $i++) {
	...
	if($sam == 1) {
		...
		&shift_sam_file($files[$i], $strains[$i], $out_name, $paired);
	} elsif($peak == 1) {
	...
	#	print OUT $split[0] . "\t" . $split[1] . "\t" . $split[2] . "\t" . &shift($chr, $allele, $split[3]);
		...
		if($pair == 1) {
			if(!exists $last{$chr}{$allele}) {
				...
				$coord = &shift($chr, $allele, $split[7]);
			}
			print $out "\t" . $split[4] . "\t" . $split[5] . "\t" . $split[6] . "\t" . $coord;
			for(my $j = 8; $j < @split; $j++) {
				print $out "\t" . $split[$j];
			}
		} else {
